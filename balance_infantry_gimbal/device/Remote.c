...
{
    if ((rc_ctrl.key.v & Key) == 0 && (TuRc.key.v & Key) == 0)
        return 0;
...
bool_t CheakKeyPressOnce(uint16_t Key)
{
    ...
    if ((KeyJumpChannal & Key) == 0 && ((TuRc.KeyJumpChannal & Key) == 0)){
        return 0;
    }
    else{
        if ((KeyUsed & Key) == 0) {
...
fp32 MouseMoveX()
{
    return ((rc_ctrl.mouse.x|TuRc.mouse.x) / 32768.0f);
}
fp32 MouseMoveY()
{
    return ((rc_ctrl.mouse.y|TuRc.mouse.y) / 32768.0f);
}

// 鼠标左右键
bool_t MousePressLeft()
{
    return rc_ctrl.mouse.press_l|TuRc.mouse.press_l;
}
bool_t MousePressRight()
{
    return rc_ctrl.mouse.press_r|TuRc.mouse.press_r;
}
