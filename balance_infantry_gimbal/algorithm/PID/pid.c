...
fp32 l_t;
fp32 kys=1.0f;
fp32 cascade_PID_calc_yaw(cascade_pid_t *c_pid, fp32 o_ref, fp32 i_ref, fp32 o_set,fp32 i_set)
{
    c_pid->s_set = o_set;
    c_pid->s_fdb = o_ref;
    c_pid->v_fdb = i_ref;
    c_pid->v_set = yaw_PID_calc(&c_pid->pid_outside, c_pid->s_fdb, c_pid->s_set);
    c_pid->out = yaw_PID_calc(&c_pid->pid_inside, c_pid->v_fdb, c_pid->v_set+i_set*kys);
	  l_t=o_set;
    if(c_pid->v_set+i_set>c_pid->v_fdb)	c_pid->out+=500;
		if(c_pid->v_set+i_set<c_pid->v_fdb)	c_pid->out-=500;
    return c_pid->out;
}
